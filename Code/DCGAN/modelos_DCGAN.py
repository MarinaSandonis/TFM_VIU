...
def Discriminator(input_shape,layers_num, num_filters, size_kernel, stride_size, dropout_rate, use_batch_norm = False, use_dropout=False): 
  ...
  for i in range(0, layers_num): 
      ...
      if i==layers_num-1:
          x = Conv2D(1,kernel_size=size_kernel,strides=stride_size,padding="same",use_bias = False,activation = 'sigmoid')(x)
...
def Generator(latent_dim, layers_num, num_filters, size_kernel,stride_size, img_shape, use_batch_norm=False):
    ...
    output_size = (img_shape[0] // (2**layers_num)) * (img_shape[0] // (2**layers_num)) * (2 * num_filters * (2 ** (layers_num - 1)))
    ...
    for i in range(0, layers_num):
        ...
        if use_batch_norm & i!=0:
            x = BatchNormalization()(x)
        x = LeakyReLU(alpha=0.2)(x)
    generator_output = Conv2DTranspose(img_shape[2], kernel_size=size_kernel, strides =1 , activation='tanh', padding='same')(x)
